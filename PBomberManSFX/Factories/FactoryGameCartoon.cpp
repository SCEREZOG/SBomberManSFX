...
{
    // spawn player
    auto player = std::make_shared<CartoonPlayer>(gameManager->getAssetManager()->getTexture(Texture::Player_Cartoon), gameManager->getRenderer());
...
std::shared_ptr<GameObject> FactoryGameCartoon::CreateEnemy(AIType _type, const int _positionX, const int _positionY)
{
    ...
    auto enemy = std::make_shared<CartoonEnemy>(gameManager->getAssetManager()->getTexture(Texture::Enemy1_Cartoon), gameManager->getRenderer());
