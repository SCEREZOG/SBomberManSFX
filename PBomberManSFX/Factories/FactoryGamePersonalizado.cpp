...
{
    // spawn player
    auto player = std::make_shared<PersonalizadoPlayer>(gameManager->getAssetManager()->getTexture(Texture::Player_Personalizado), gameManager->getRenderer()); 
...
std::shared_ptr<GameObject> FactoryGamePersonalizado::CreateEnemy(AIType _type, const int _positionX, const int _positionY)
{
    ...
    auto enemy = std::make_shared<PersonalizadoEnemy>(gameManager->getAssetManager()->getTexture(Texture::Enemy1_Personalizado), gameManager->getRenderer());
